...
def find_files(filename, search_top_dir):
    ...
    for root_dir, dir_local, files in os.walk(search_top_dir):
        if filename in files:
            ...
            #logger.debug(f"FIND_FILES: Found: {os.path.join(root_dir,filename)}")
...
def generate_sim_files():
    ...
    sim_files  = f"sh {sim_files_path} d5005"
...
def create_test_list():
    ...
    if (args.package == "list"):
        test_dir_pattern = r'^\.\/(\w+)\/scripts\/run_sim\.sh'
        try:
        ...
        for test in working_test_list:
            ...
            if (test_dir_pattern_found):
                ...
                unit_test_script = top_test_dir + "/" + test_dir + "/" + "scripts" + "/" + "run_sim.sh"
        ...
        if (args.package == "all"):
            for test in working_test_list:
                ...
                filtered_test_list.append(test_path_split[-2])
        else:
            test_dir_pattern = "\/(" + args.package + r'\w+)\/scripts$'
...
def scan_test_results(test_list):
    ...
    for test in test_list:
        test_name = test
        sim_dir = top_test_dir + "/" + test + "/" + "scripts" + "/" + f"sim_{args.simulator}"
...
def sim_process(index, test, test_dir_top, simulator):
    ...
    test = test.replace('\n', '')
    test_dir = test_dir_top + '/' + test + '/' + "scripts"
    test_file = test_dir_top + '/' + test + "/" + "scripts" + "/" + "run_sim.sh"
    logger.info(f"   Process {index_string} for test <{test_name_extracted:.<{longest_test_name}}> date/time started....: {sim_start}")
    logger.info(f"   Test Dir : {test_dir}")
    if (os.path.exists(test_file)):
        os.chdir(test_dir)
        if (simulator == 'vcs'):
            sim_command_line  = f"sh run_sim.sh"
        elif (simulator == 'vcsmx'):
            sim_command_line  = f"sh run_sim.sh VCSMX=1"
        else:
            sim_command_line  = f"sh run_sim.sh MSIM=1"
...
def sim_farm_process(index, test, test_dir_top, simulator):
    ...
    if (os.path.exists(test_file)):
        os.chdir(test_dir)
        if (simulator == 'vcs'):
            arc_submit_command_line = f"arc submit -PE flow/sw/bigmem mem=20000 -- sh run_sim.sh"
        elif (simulator == 'vcsmx'):
            arc_submit_command_line = f"arc submit -PE flow/sw/bigmem mem=20000 -- sh run_sim.sh VCSMX=1"
        else:
            arc_submit_command_line = f"arc submit -PE flow/sw/bigmem mem=20000 -- sh run_sim.sh MSIM=1"
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
            ...
            description=textwrap.dedent('''Unit Test Simulation Regression Test Runner'''),
    ...
    parser.add_argument('-k', '--pack', dest='package', type=str, nargs='?', default='all', choices=['all','dfh','fme','he_hssi','he_lb','he_mem','list'], help='Test package to run during regression.  The "list" option will look for a text file named "list.txt" in the "unit_test" directory for a text list of tests to run (top directory names).  (Default: %(default)s)')
    ...
    if (total_processes_to_run > 0):
        if args.run_regression_locally:
            ...
            test_items = [pool.apply_async(sim_process, (i, list_of_tests[i], top_test_dir, args.simulator)) for i in range(total_processes_to_run)]
            logger.info(f"Unit Test Pool Launch Completed.")
            ...
            test_items = [pool.apply_async(sim_farm_process, (i, list_of_tests[i], top_test_dir, args.simulator)) for i in range(total_processes_to_run)]
            logger.info(f"Unit Test Farm Test Pool Launch Completed.")
            for item in test_items:
                item.get()
            while not queue.empty():
        ...
        scan_test_results(list_of_tests)
