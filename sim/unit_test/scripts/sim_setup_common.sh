...
cp ${IP_SIM_SCRIPT_DIR}/../rtl_comb.f $SIM_DIR


if [ $MSIM -eq 1 ]; then
  sed -i 's/PLAT_RTL_FILELIST=.*/PLAT_RTL_FILELIST="-f .\/rtl_comb.f"/' ${SIM_DIR}/msim_filelist.sh
else
  sed -i 's/PLAT_RTL_FILELIST=.*/PLAT_RTL_FILELIST="-f .\/rtl_comb.f"/' ${SIM_DIR}/vcs_filelist.sh
fi
sed -i 's,'.*\/pcie_top.sv',$OFS_ROOTDIR/sim/bfm/pcie_top.sv,' ${SIM_DIR}/rtl_comb.f
