...
#    Stratix 10 Partial Reconfiguration Flow Script
#
#    This template, once configured for your design, can be used to invoke the
#    partial reconfiguration flow for an Stratix 10 design. To use this template
...
#
#    You can run this script using qpro_sh -t ofs_partial_reconfig/flow.tcl
#
...
proc initialize {skip_base_check} {
	...
	if {($skip_base_check == 0)} {
		...
		if {[lsearch -exact [list "STRATIX 10"] [string toupper [get_dstr_string -family $value]]] == -1} {
			post_message -type error "The current family $value ([get_dstr_string -family $value]) is not supported by the ofs_partial_reconfig flow script. This script only supports families: Stratix 10."
			qexit -error
		}
...
proc print_pr_project_info {} {
	...
	puts "Stratix 10 Partial Reconfiguration Flow"
	puts "-------------------------------------------------------------------------------"
...
proc main {} {
	...
	if {$options(check)} {
		...
		post_message -type info "Successfully completed IOFS PR compile."
...
if {($::quartus(nameofexecutable) == "quartus") || ($::quartus(nameofexecutable) == "quartus_pro") || ($::quartus(nameofexecutable) == "qpro")} {
	...
	post_message -type error "The Stratix 10 PR compile flow script can only be run from the Quartus GUI or quartus_sh."
}
