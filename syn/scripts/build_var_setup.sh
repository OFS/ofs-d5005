...
# 
#cd ${OFS_ROOTDIR}/src/pd_qsys/d5005/fabric
#sh gen_fabrics.sh
#cd ${OFS_ROOTDIR}
...
if [ ! -z ${BUILD_VAR_SETUP_COMPLETE} ]; then
    ...
    if [ ${BUILD_VAR_SETUP_COMPLETE} == "1" ]; then
        ...
        echo "  SIGNALTAP_ENABLE_SCRIPT_SH_FILE = ${SIGNALTAP_ENABLE_SCRIPT_SH_FILE}"
...
export USER_CLOCK_PATTERN="*user_clk_iopll|iopll_0_outclk*"
...
#FUTURE_IMPROVEMENT:  setup IP_LIB script may be temporary (only for ofs_ac project)
...
export PMCI_NIOS_HEX_FILE=${OFS_ROOTDIR}/${OFS_PROJECT}/ipss/pmci/pmci_ss_nios_fw.hex
...
export WORK_POST_COMPILE_SCRIPT_SH_FILE=${WORK_DIR}/ofs-common/scripts/common/syn/post_compile_script.sh
export BUILD_FIM_LOG_FILE=${OFS_ROOTDIR}/build_fim_$(basename ${WORK_DIR}).log
# TODO:temporary fix for n6001 (remove once qsf and design_files.tcl points to the correct IP/IPSS dir.
export WORK_SETUP_IP_LIB_SH_FILE=${WORK_DIR}/ofs-common/scripts/common/syn/setup_ip_lib.sh
...
export PIM_PLATFORM_NAME=ofs_d5005
export PIM_INI_FILE=${OFS_ROOTDIR}/src/top/${PIM_PLATFORM_NAME}.ini
export PIM_DEFAULT_AFU=${OFS_ROOTDIR}/src/port_gasket/afu_main_pim/std_exerciser/filelist.txt
...
if [ ! -z ${OFS_BOARD_VARIANT} ]; then
    if [[ ${OFS_BOARD_VARIANT,,} == *"signaltap"* ]]; then
        export SIGNALTAP_ENABLE_SCRIPT_SH_FILE="${WORK_DIR}/ofs-common/scripts/common/syn/signaltap_enable.sh"
    fi
fi
