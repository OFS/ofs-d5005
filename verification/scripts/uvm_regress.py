...
def set_top_list_of_tests():
    top_list_of_tests = rootdir + "/verification/tests/test_pkg.svh"
    if os.path.exists(top_list_of_tests):
        logger.debug(f"      List of UVM tests found.....:{top_list_of_tests}")
    else:
        logger.error(f"ERROR: List of UVM tests not found at:{top_list_of_tests}.  Please check your repo/environment to continue.")
        logger.error(f"       Script {os.path.basename(__file__)} execution has been halted.") 
        sys.exit(1)
...
def send_email_report():
    ...
    message["Subject"] = f"[D5005] UVM Regression results - Tool:{args.simulator}"
...
def scan_test_results(test_list,working_dir, simulator):
    ...
    if(simulator == 'vcs'):
      ...
      sim_dir = "/".join(working_dir_list) + "/sim_msim"
      uvm_error_pattern = r'UVM_ERROR\s*:\s*(\d+)'
      uvm_fatal_pattern = r'UVM_FATAL\s*:\s*(\d+)'
...
def build_sim_env(coverage, simulator):
    ...
    if coverage:
        if (simulator == 'vcs'):
            cmplib_command_line = None
            build_command_line  = f"gmake -f {makefile} {build} COV=1"
        else:
            cmplib_command_line = None
            build_command_line  = None
            logger.error(f"ERROR: Code Coverage is currently only supported with the Synopsys VCS simulator.")
            sys.exit(1)
    else:
        if (simulator == 'vcs'):
            cmplib_command_line = None
            build_command_line  = f"gmake -f {makefile} {build}"
        else:
    ...
    if (cmplib_command_line):
        ...
        if (cmplib_cmd_success == 0):
            ...
            logger.error(f"ERROR: Make Library Compile Command was not successful. Returned:cmplib_cmd_success:{cmplib_cmd_success}")
            sys.exit(1)
    if (build_command_line):
        ...
        if (build_cmd_success == 0):
            ...
            logger.error(f"ERROR: Make Build Command was not successful. Returned:build_cmd_success:{build_cmd_success}")
            sys.exit(1)
...
def sim_process(index, test, test_dir_top, coverage, simulator):
    makefile = "Makefile_" + simulator.upper() + '.mk'
    test_name_pattern = r'(\w+)\.svh'
    test_name_valid = re.search(test_name_pattern, test)
    ...
    if (test_name_valid):
        ...
        if (os.path.exists(test_file)):
            ...
            if coverage:
                if (simulator == 'vcs'):
                    sim_command_line = f"gmake -f {makefile} {testname} run COV=1"
                else:
                    sim_command_line = f"gmake -f {makefile} {testname} run"
                    logger.error(f"WARNING: Code Coverage is currently only supported with the Synopsys VCS simulator.")
            else:
                sim_command_line = f"gmake -f {makefile} {testname} run"
...
def sim_farm_process(index, test, test_dir_top, coverage, simulator):
    makefile = "Makefile_" + simulator.upper() + '.mk'
    ...
    if (test_name_valid):
        ...
        if (os.path.exists(test_file)):
            ...
            if coverage:
                if (simulator == 'vcs'):
                    arc_submit_command_line = f"arc submit -PE flow/sw/bigmem mem=20000 -- gmake -f {makefile} {testname} run COV=1"
                else:
                    arc_submit_command_line = f"arc submit -PE flow/sw/bigmem mem=20000 -- gmake -f {makefile} {testname} run"
                    logger.error(f"WARNING: Code Coverage is currently only supported with the Synopsys VCS simulator.")
            else:
                arc_submit_command_line = f"arc submit -PE flow/sw/bigmem mem=20000 -- gmake -f {makefile} {testname} run"
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
            ...
            description=textwrap.dedent('''UVM Regression Test Runner'''),
            epilog=textwrap.dedent('''\
            Example: below performs a UVM regression test, run locally, with 8 processes, using VCS with code coverage.
               python uvm_regress.py -l -n 8 -s vcs -c
            Same as above using long-form args: 
               python uvm_regress.py --local --n_procs 8 --sim vcs --cov
            Same as above, but run on computing Farm (no --local): 
               python uvm_regress.py --sim vcs --cov
            Running script using defaults: run on computing Farm, using VCS with no code coverage:
               python uvm_regress.py'''))
    parser.add_argument('-l', '--local', dest='run_regression_locally', action='store_true', help='Run regression locally, or run it on a computing farm resource.  (Default: %(default)s)')
    ...
    if not args.bypass_library_build:
        build_sim_env(args.coverage, args.simulator)
    ...
    if args.run_regression_locally:
        ...
        test_items = [pool.apply_async(sim_process, (i, list_of_tests[i], test_dir, args.coverage, args.simulator)) for i in range(total_processes_to_run)]
        ...
        test_items = [pool.apply_async(sim_farm_process, (i, list_of_tests[i], test_dir, args.coverage, args.simulator)) for i in range(total_processes_to_run)]
    ...
    scan_test_results(list_of_tests, current_dir, args.simulator)
