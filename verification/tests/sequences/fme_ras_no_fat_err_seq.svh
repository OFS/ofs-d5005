...
// This sequnce verifies the RAS no_fatal error regsiter of FME  
  ...
  constraint err_type_cons{
	...
	bit [31:0] host_intr_rdata[16];
      ...
      `uvm_info(get_name(), $psprintf("TEST: STEP 0 - inj_ras_err=%0d, inj_fme_err=%0d dut_mem_start=%0h dut_mem_end=%0h", inj_ras_err, inj_fme_err, dut_mem_start, dut_mem_end), UVM_LOW)
          ...
          mmio_write64(.addr_(pf0_bar4+'h3060), .data_(intr_addr));
          #1us;
          ...
          mmio_write64(.addr_(pf0_bar4+'h3068), .data_(intr_masked_data));
          #25us;
 
	  if(inj_ras_err)begin
             uvm_status_e      status;
            `uvm_info(get_name(), $psprintf("TEST: STEP 2 - Inject RAS ERROR ras_err_code=%0x",'h4), UVM_LOW)
             tb_env0.fme_regs.RAS_ERROR_INJ.write(status,'h4);
	  end
	     ...
	     @(posedge `MSIX_TOP.o_msix_valid)
          ...
          for(int i=0;i<200;i++) begin
            mmio_read64(.addr_(pf0_bar4+'h2000),.data_(rdata));
            if(rdata[6]) break;
            #1ns;
          end
          assert(rdata[6]) else 
          ...
          mmio_write64(.addr_(pf0_bar4+'h3068), .data_(intr_wr_data));
